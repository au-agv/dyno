...
:::{caution}
Only the `devel` and `runtime` prebuilt images are directly available from the
GitHub container registry due to the licensing restrictions of the NVIDIA OptiX
library bundled in the other tagged images.

Note that you can still build these images locally from source.
:::
