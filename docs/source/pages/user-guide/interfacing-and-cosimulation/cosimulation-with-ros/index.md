...
The teleop node provides operation via keyboard by serialising keyboard input to
a drive-by-wire message with throttle, brake and steer inputs.

The teleop node must be launched with a valid terminal where keyboard events can
be captures. This can be achieved by launching the node using `ros2 run` from a
separate terminal window or, alternatively, using a launch file by using the
`prefix="xterm -e"` option for the node, which will spawn a separate terminal
window at launch.
